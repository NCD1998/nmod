...
public class MysticTree {
	private IBlockState[][][] blocks = new IBlockState[5][5][10];
	Random Rand = new Random();
	
	
	public MysticTree(BlockPos pos){
		...
		for(int x = 0; x < 5; x++){
			for(int z = 0; z < 5; z++){
				if(!(((z == 4) && ((x == 0) || (x == 4))) || (z == 0 && x == 4))){
					blocks[x][z][4 + heightInc] = NBlocks.MysticLeaves.getDefaultState();
				}
			}
		}
		for(int x = 0; x < 5; x++){
			for(int z = 0; z < 5; z++){
				if(!(x == 0 && z == 0)){
					blocks[x][z][5 + heightInc] = NBlocks.MysticLeaves.getDefaultState();
				}
			}
		}
		for(int x = 1; x < 4; x++){
			for(int z = 1; z < 4; z++){
				if(!(x == 1 && z == 1) && !(x == 3 && z == 1)){
					blocks[x][z][6 + heightInc] = NBlocks.MysticLeaves.getDefaultState();
				}
			}
		}
		for(int x = 1; x < 4; x++){
			for(int z = 1; z < 4; z++){
				if(!(x == 1 && z == 1) && !(x == 3 && z == 1) && !(x == 1 && z == 3) && !(x == 3 && z == 3)){
					blocks[x][z][7 + heightInc] = NBlocks.MysticLeaves.getDefaultState();
				}
			}
		}
		for(int x = 0; x < height; x++){
			blocks[2][2][x] = NBlocks.MysticWoodLog.getDefaultState();
		}
	}
